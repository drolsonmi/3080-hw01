# YOUR NAME
# Math 3080
# DATE OF COMPLETION
# problem_1.py

#####   PROBLEM   #####
# You are dining at the "Budget Bistro." Your program should:
# Ask the user for the original cost of the meal.
# Ask the user for the tip percentage they wish to leave (e.g., 15).
# Calculate the tip amount and the total bill.
# Print a summary of the costs.

#####   CODE   #####

def calculate_bill():
    # 1. Get input from the user for the meal cost (float)
    # 2. Get input from the user for the tip percentage (int)
    
    # 3. Calculate the tip amount
    # 4. Calculate the total bill
    
    # 5. Print the results formatted to 2 decimal places
    pass

if __name__ == "__main__":
    calculate_bill()
